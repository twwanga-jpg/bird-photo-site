...
function run(argv){
  ...
  const photos=lines.map((line,index)=>{
    const p=line.split('\t');
    return {id:p[0],category:p[1],src:p[3],small:p[2],large:p[4],title:clean(p[5])||`作品 ${index+1}`,species:clean(p[6]),location:clean(p[7]),alt:[p[5],p[6],p[7]].filter(Boolean).join('，'),featured:index===0};
  });
